...
namespace PaparazziGroundControlStation.Util
{

    public class GMapPlane : GMarkerGoogle
    {
        Image Plane;
        ...
        private bool ShowBallon;
        public GMapPlane(GMap.NET.PointLatLng p, Image Plane, bool ShowBallon)
            : base(p, new Bitmap(Plane))
        {
            Size = new Size(32, 32);
            Offset = new Point(-Size.Width / 2, -Size.Height / 2);
            LocalPosition = new Point(LocalPosition.X + Offset.X, LocalPosition.Y + Offset.Y);
            this.Plane = Plane;
            this.ShowBallon = ShowBallon;
        }
        public void DrawBallon(Graphics g, Rectangle bounds, int radius)
        {
            ...
            g.DrawString("Bearing: " + Parameters.Pbearing.ToString("0")+"\n" + "Distance: " + Parameters.Navigation.dist_home.ToString("0") , new Font("Tahoma", 10, FontStyle.Italic), Brushes.White, bounds.Left + 10, bounds.Top + 10);
            if (radius == 0)
            {
                path.AddRectangle(bounds);
                g.FillPath(new SolidBrush(Color.FromArgb(100, 80, 80, 80)), path);
            }
        ...
        {
            Matrix m = g.Transform; // Push Matrix
            g.TranslateTransform(LocalPosition.X - Offset.X, LocalPosition.Y - Offset.Y);
            g.RotateTransform((float)Angle - 90);
            g.ScaleTransform(Size.Width / (float)Plane.Width, Size.Height / (float)Plane.Height);
            g.DrawImage(Plane, new PointF(-Plane.Width / 2 - Offset.X, -Plane.Width / 2 - Offset.Y));
            g.Transform = m; // Pop Matrix :D
            if (ShowBallon)
                DrawBallon(g, new Rectangle(LocalPosition.X - Offset.X + 20, LocalPosition.Y - Offset.Y - 60, 150, 50), 10);
    ...
    {
        public UTMConvertor(String datumNameIn)
        {
    ...
    public class GMapStationMarker : GMarkerGoogle
    {
        public double Alt = 0;
        ...
        public GMapStationMarker(GMap.NET.PointLatLng p, string name)
        ...
        public GMapStationMarker(GMap.NET.PointLatLng p, string name, double radius)
            : base(p, GMarkerGoogleType.blue)
        {
            Size = new Size(32, 32);
            Offset = new Point(-Size.Width / 2, -Size.Height / 2);
        ...
        {
            base.OnRender(g);
            Point TextPosition = LocalPosition;
            TextPosition.X += 12 - name.Length * 2;
            TextPosition.Y += 1;
            g.DrawString(name, new Font("Tahoma", 8, FontStyle.Regular, GraphicsUnit.Point), Brushes.Black, TextPosition);
            if (ShowCircle)
                g.FillEllipse(new SolidBrush(Color.FromArgb(80, Color.Red)), LocalPosition.X - (int)circleRadius / 2, LocalPosition.Y - (int)circleRadius / 2, (int)circleRadius, (int)circleRadius);
        }
    }
